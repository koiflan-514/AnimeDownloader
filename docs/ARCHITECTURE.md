...
- **主窗口**：左侧 56px 手绘导轨（**不是 `NavigationView`**）承载三页导航（画廊 / 灯箱 / 设置）；
  顶部命令条集中放置图源下拉（图标 + 名称 + 描述）、分段式 NSFW 过滤（`SegmentedControl`）、
  状态区（缩略图 + 状态点 + 可截断的状态文本）与刷新 / 清理缓存按钮。
  状态点三档：就绪（灰，无光源）/ 工作（强调色）/ 故障（红），页面通过 `SetGlobalStatus` 同步。
  导轨底部是快捷键速查浮层入口与应用标记；标题栏显示当前图片的缩略图预览。
  `App.xaml` 定义亮/暗两套主题字典，强调色跟随系统 `SystemAccentColor*`。
- **全屏与外壳**：F11 走窗口级 `KeyboardAccelerator`（与焦点无关），由 `MainWindow.SetFullscreen`
  统一处理三层 —— 窗口呈现器、外壳（导轨列宽归零 + 命令条）、页内控件（`IImmersivePage.SetImmersive`）。
  Esc 走根部 `KeyDown` 且只在全屏时接管，避免被 `ComboBox` / 弹层消化掉的 Esc 误触发退出全屏。
...
- **查看器页**：沉浸式深色查看区（亮暗主题均保持深色，突出图片），底部浮动工具栏
  ...
  键盘快捷键（空格 / 右箭头换图、Ctrl+S 保存、Ctrl+0 适应屏幕、Esc 退出全屏）、
  来源链接、保存进度。**F11 全屏统一交给外壳**（页面实现 `IImmersivePage`），
  这样进全屏时导轨与命令条会一起收起，退出时窗口几何与最大化状态也能被还原。
...
- **集中式样式**：`App.xaml` 定义亮/暗两套主题字典（`App*Brush` 系列：表面色阶、文本、强调、
  状态色、浮动工具栏、遮罩等）。`AppAccent*` **不含硬编码色值**，全部指向 Fluent 的 `SystemAccentColor*`，
  由 `WindowsColorScheme` 服务读取 `UISettings` 回写，并按 WCAG 相对亮度（阈值 0.4）自动取黑/白墨色；
  `Styles/AppStyles.xaml` 提供圆角令牌、卡片/面板/状态栏样式、胶囊按钮、分段切换、
  圆形图标按钮（自定义模板）、状态点样式等；`Styles/FluentTheme.xaml` 覆写 Fluent 的中性基线画刷
  （必须在 `XamlControlsResources` 之后合并，否则同名键不生效）；全部通过 `ThemeResource` 引用，支持亮 / 暗主题。
